...
public:
    ...
    bool solve(const BlockSparseMatrix<MatrixType>& matrix, const std::vector<VectorType>& rhs, std::vector<VectorType>& result, ScalarType& residual_out, int& iterations_out)
    {
        ...
        residual_out = ParallelBLAS::abs_max<ScalarType, VectorType>(r);
        ...
        ScalarType rho = ParallelBLAS::dot<ScalarType, VectorType>(r, r);
        ...
        for(int iteration = 0; iteration < m_MaxIterations; ++iteration)
        {
            ...
            ScalarType tmp = ParallelBLAS::dot<ScalarType, VectorType>(s, z);
            ...
            ScalarType rho_new = ParallelBLAS::dot<ScalarType, VectorType>(r, r);
    ...
    bool solve_precond(const BlockSparseMatrix<MatrixType>& matrix, const std::vector<VectorType>& rhs, std::vector<VectorType>& result, ScalarType& residual_out, int& iterations_out)
    {
        ...
        ParallelBLAS::add_scaled<ScalarType, VectorType>(-1.0, s, r);
        ...
        ScalarType rho = ParallelBLAS::dot<ScalarType, VectorType>(z, r);
        ...
        for(int iteration = 0; iteration < m_MaxIterations; ++iteration)
        {
            ...
            ScalarType alpha = rho / ParallelBLAS::dot<ScalarType, VectorType>(s, z);
            tbb::parallel_invoke(
                [&] {
                ParallelBLAS::add_scaled<ScalarType, VectorType>(alpha, s, result);
            },
                [&] {
                ParallelBLAS::add_scaled<ScalarType, VectorType>(-alpha, z, r);
            });
            ...
            ScalarType rho_new = ParallelBLAS::dot<ScalarType, VectorType>(z, r);
            ScalarType beta    = rho_new / rho;
            ParallelBLAS::add_scaled<ScalarType, VectorType>(beta, s, z);
            s.swap(z); // s=beta*s+z
            rho = rho_new;
        }
...
private:
    void form_preconditioner(const BlockSparseMatrix<MatrixType>& matrix, std::vector<MatrixType>& diag)
    {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, matrix.size), [&](tbb::blocked_range<UInt32> r)
        {
