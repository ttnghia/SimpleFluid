...
struct BlockSparseMatrix
{
    ...
    explicit BlockSparseMatrix(UInt32 n = 0)
        : size(n), m_ColIndex(n), m_ColValue(n)
    {}
    ...
    {
        auto iter = std::lower_bound(m_ColIndex[i].begin(), m_ColIndex[i].end(), j);
    ...
    void symmetric_remove_row_and_column(UInt32 i)
    {
        for(UInt32 a = 0; a < m_ColIndex[i].size(); ++a)
        {
            ...
            for(UInt32 b = 0; b < m_ColIndex[j].size(); ++b)
            {
                if(m_ColIndex[j][b] == i)
                {
                    MathUtils::erase(m_ColIndex[j], b);
                    MathUtils::erase(m_ColValue[j], b);
                    break;
                }
            }
        }
              ...
              std::vector<VectorType>&             result)
{
        ...
        [&](tbb::blocked_range<size_t> r)
    {
        for(size_t i = r.begin(); i != r.end(); ++i)
        {
            ...
            for(size_t j = 0; j < matrix.m_ColIndex[i].size(); ++j)
            {
                ...
                BlockSparseMatrix<MatrixType>&       matrix)
{
        ...
        [&](tbb::blocked_range<size_t> r)
    {
        for(size_t i = r.begin(); i != r.end(); ++i)
        {
            ...
            for(size_t j = 0; j < A.m_ColIndex[i].size(); ++j)
            {
...
struct BlockFixedSparseMatrix
{
    ...
    void construct_from_matrix(const BlockSparseMatrix<MatrixType>& matrix)
    {
        ...
        for(UInt32 i = 0; i < size; ++i)
        {
            rowstart[i + 1] = rowstart[i] + matrix.m_ColIndex[i].size();
        }
            ...
            [&](tbb::blocked_range<size_t> r)
        {
            for(size_t i = r.begin(); i != r.end(); ++i)
            {
                for(UInt32 k = 0; k < matrix.m_ColIndex[i].size(); ++k)
                {
                    value[rowstart[i] + k] = matrix.m_ColValue[i][k];
                    colindex[rowstart[i] + k] = matrix.m_ColIndex[i][k];
                }
            }
        }, ap); // end parallel_for
    }
